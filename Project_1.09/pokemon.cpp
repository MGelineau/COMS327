...
int pokemon::get_move_accuracy(int i) const{
...
bool pokemon::stab(int i) const{
  return moves[move_index[i]].type_id == pokemon_types[pokemon_index].type_id;
}
...
void pokemon::set_health(int i){
  hp = i;
}

int pokemon::get_power(int i) const{
  return moves[move_index[i]].power;
}

int pokemon::get_poke_speed() const{
