...
void fight(character_t *nerd)
{
  nerd->npc->massacred = 1;
}
...
void enterBuilding()
{
  ...
  clrtoeol();
  print_map();
  mvprintw(0, 0, "Entering Building...");
}

int searchPlayer(int x, int y)
{
  // Checks all positions around current pc for areas they cannot move
  if (
      ...
      ((world.cur_map->cmap[world.pc.pos[dim_y] + y][world.pc.pos[dim_x] + x]) &&
       (world.cur_map->cmap[world.pc.pos[dim_y] + y][world.pc.pos[dim_x] + x]->npc->massacred == 1)))
  { // Checks if this spot has a defeated PC
    return 2;
  }
  else if (world.cur_map->cmap[world.pc.pos[dim_y] + y][world.pc.pos[dim_x] + x])
  { // If PC is not defeated, then fight
    clrtoeol();
    mvprintw(0, 0, "Get ready for a tussle");
    fight(world.cur_map->cmap[world.pc.pos[dim_y] + y][world.pc.pos[dim_x] + x]);
    return 3;
  }
  return 1;
}
...
int movePc()
{
  ...
  case '7':
  case 'y':
    if (searchPlayer(-1, -1) > 1)
    {
      return searchPlayer(-1, -1);
    }
    else
    {
  ...
  case '6':
  case 'l':
    if (searchPlayer(1, 0) > 1)
    {
      return searchPlayer(1, 0);
    }
    else
    {
      world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
...
void game_loop()
{
  ...
  while (loop)
  {
    character = heap_remove_min(&world.cur_map->turn);
    if (character == &world.pc)
    {
      print_map();
      ...
      pathfind(world.cur_map);
      character->next_turn += move_cost[char_pc][world.cur_map->map[character->pos[dim_y]]
                                                           [character->pos[dim_x]]];
    }
    else
    {
      move_func[character->npc->mtype](character, pair);
      if (character->npc->massacred == 1)
      {
        character->next_turn += move_cost[character->npc->ctype][world.cur_map->map[character->pos[dim_y]]
                                                                   [character->pos[dim_x]]];
      }
      else
      {
        world.cur_map->cmap[character->pos[dim_y]][character->pos[dim_x]] = NULL;
        world.cur_map->cmap[pair[dim_y]][pair[dim_x]] = character;
        character->next_turn += move_cost[character->npc->ctype][world.cur_map->map[pair[dim_y]]
                                                                   [pair[dim_x]]];
        character->pos[dim_y] = pair[dim_y];
        character->pos[dim_x] = pair[dim_x];
      }
    }
    heap_insert(&world.cur_map->turn, character);
  }
}
