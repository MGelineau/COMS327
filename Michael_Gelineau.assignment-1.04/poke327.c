...
#define min(x, y) \
  ({__typeof__ (x) _x = (x);\
 __typeof__ (y) _y = (y); \
 _x < _y ? _x : _y; }) // Always parens around function-type macros
...
#define min8(a, b, c, d, e, f, g, h)                                   \
  ({                                                                   \
    ...
    __typeof__(g) _g = (g);                                            \
    __typeof__(h) _h = (h);                                            \
    min(min(min(min(min(min(min(_a, _b), _c), _d), _e), _f), _g), _h); \
  })
...
static void print_map()
{
  ...
  for (y = 0; y < MAP_Y; y++)
  {
    for (x = 0; x < MAP_X; x++)
    {
      ...
      if (world.cur_map->trainers.hiker.pos[dim_y] == y && world.cur_map->trainers.hiker.pos[dim_x] == x && countHiker != 1)
      {
        putchar('h');
        countHiker++;
      }
      if (world.cur_map->trainers.rival.pos[dim_y] == y && world.cur_map->trainers.rival.pos[dim_x] == x && countRival != 1)
      {
        putchar('r');
        countRival++;
      }
...
void move_Trainers()
{
  int hPosY = world.cur_map->trainers.hiker.pos[dim_y];
  int hPosX = world.cur_map->trainers.hiker.pos[dim_x];
  int rPosY = world.cur_map->trainers.rival.pos[dim_y];
  int rPosX = world.cur_map->trainers.rival.pos[dim_x];

  int h1 = world.hiker_dist[hPosY + 1][hPosX];
  int h2 = world.hiker_dist[hPosY - 1][hPosX];
  int h3 = world.hiker_dist[hPosY + 1][hPosX - 1];
  int h4 = world.hiker_dist[hPosY + 1][hPosX + 1];
  int h5 = world.hiker_dist[hPosY - 1][hPosX + 1];
  int h6 = world.hiker_dist[hPosY - 1][hPosX - 1];
  int h7 = world.hiker_dist[hPosY][hPosX + 1];
  int h8 = world.hiker_dist[hPosY][hPosX - 1];

  int r1 = world.rival_dist[rPosY + 1][rPosX];
  int r2 = world.rival_dist[rPosY - 1][rPosX];
  int r3 = world.rival_dist[rPosY + 1][rPosX - 1];
  int r4 = world.rival_dist[rPosY + 1][rPosX + 1];
  int r5 = world.rival_dist[rPosY - 1][rPosX + 1];
  int r6 = world.rival_dist[rPosY - 1][rPosX - 1];
  int r7 = world.rival_dist[rPosY][rPosX + 1];
  int r8 = world.rival_dist[rPosY][rPosX - 1];

  int hSpot = min8(h1, h2, h3, h4, h5, h6, h7, h8);
  int rSpot = min8(r1, r2, r3, r4, r5, r6, r7, r8);

  if (world.hiker_dist[hPosY + 1][hPosX] == hSpot)
  {
    world.cur_map->trainers.hiker.pos[dim_y] = world.cur_map->trainers.hiker.pos[dim_y + 1];
  }

  else if (world.hiker_dist[hPosY - 1][hPosX] == hSpot)
  {
    world.cur_map->trainers.hiker.pos[dim_y] = world.cur_map->trainers.hiker.pos[dim_y - 1];
  }

  else if (world.hiker_dist[hPosY + 1][hPosX - 1] == hSpot)
  {
    world.cur_map->trainers.hiker.pos[dim_y] = world.cur_map->trainers.hiker.pos[dim_y + 1];
    world.cur_map->trainers.hiker.pos[dim_x] = world.cur_map->trainers.hiker.pos[dim_x - 1];
  }
  ...
  else if (world.hiker_dist[hPosY][hPosX + 1] == hSpot)
  {
    world.cur_map->trainers.hiker.pos[dim_x] = world.cur_map->trainers.hiker.pos[dim_x + 1];
  }
  ...
  {
    world.cur_map->trainers.hiker.pos[dim_y] = world.cur_map->trainers.hiker.pos[dim_x - 1];
  }
  ...
  if (world.rival_dist[rPosY + 1][rPosX] == rSpot)
  {
    world.cur_map->trainers.rival.pos[dim_y] = world.cur_map->trainers.rival.pos[dim_y + 1];
  }

  else if (world.rival_dist[rPosY - 1][rPosX] == rSpot)
  {
    world.cur_map->trainers.rival.pos[dim_y] = world.cur_map->trainers.rival.pos[dim_y - 1];
  }

  else if (world.rival_dist[rPosY + 1][rPosX - 1] == rSpot)
  {
    world.cur_map->trainers.rival.pos[dim_y] = world.cur_map->trainers.rival.pos[dim_y + 1];
    world.cur_map->trainers.rival.pos[dim_x] = world.cur_map->trainers.rival.pos[dim_x - 1];
  }
  ...
  else if (world.rival_dist[hPosY][rPosX + 1] == rSpot)
  {
    world.cur_map->trainers.rival.pos[dim_x] = world.cur_map->trainers.rival.pos[dim_x + 1];
  }
  ...
  {
    world.cur_map->trainers.rival.pos[dim_y] = world.cur_map->trainers.rival.pos[dim_x - 1];
  }

  print_map();
}

void generate_Trainers()
{
  int randHikerX = (rand() % 77) + 1;
  int randHikerY = (rand() % 19) + 1;

  int randRivalX = (rand() % 77) + 1;
  int randRivalY = (rand() % 19) + 1;

  world.cur_map->trainers.hiker.pos[dim_y] = randHikerY;
  world.cur_map->trainers.hiker.pos[dim_x] = randHikerX;

  world.cur_map->trainers.rival.pos[dim_y] = randRivalY;
  world.cur_map->trainers.rival.pos[dim_x] = randRivalX;
}
...
int main(int argc, char *argv[])
{
  ...
  do
  {
    printf("%s", "Move a direction, 'n' 's' 'e' 'w': ");
    scanf("%c", &c);
    switch (c)
    {
    case 'n':

      if (world.pc.pos[dim_y] - 1 != 0)
      {
        world.pc.pos[dim_y]--;
        move_Trainers();
      }
